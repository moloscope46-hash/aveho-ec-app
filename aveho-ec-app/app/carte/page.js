...
export default function CartePage() {
  ...
  // Charger Leaflet via CDN une seule fois
  useEffect(() => {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("etablissements")
      .select("id, nom, type, adresse, code_postal, ville, telephone, email, capacite, latitude, longitude")
    ...
    geolocalises.forEach(e => {
      ...
      const popupHtml = `
        <div style="min-width: 220px; font-family: 'Segoe UI', sans-serif;">
          <div style="font-weight: 700; font-size: 15px; color: #142131; margin-bottom: 4px;">${e.nom}</div>
          ...
          <>
            ...
            <Panel style={{ marginBottom: 14, padding: "12px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", fontSize: 13 }}>
                  <span><i className="ti ti-building-hospital" style={{ color: "#185FA5", marginRight: 4 }} /> <b>{geolocalises.length}</b> étab. géolocalisés</span>
                  {nonGeolocalises.length > 0 && (
                    <span style={{ color: "#8a98a8" }}>
                      <i className="ti ti-alert-triangle" style={{ color: "#EF9F27", marginRight: 4 }} /> 
                      {nonGeolocalises.length} sans coordonnées — <a href="/etablissement/fiche" style={{ color: "#185FA5", fontWeight: 600 }}>compléter</a>
                    </span>
                  )}
            ...
            <Panel style={{ padding: 0, overflow: "hidden" }}>
              ...
              <div
                ref={mapRef}
                style={{
                  height: typeof window !== "undefined" && window.innerWidth < 640 ? 420 : 600,
                  width: "100%",
                  background: "#e3e9ee",
