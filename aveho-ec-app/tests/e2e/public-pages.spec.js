...
test.describe("Pages publiques sans authentification", () => {
  test("/mentions-legales accessible et bien structurée", async ({ page }) => {
    ...
    // Vérifie la présence des 5 sections principales
    await expect(page.locator("text=/Éditeur/i")).toBeVisible();
    await expect(page.locator("text=/Protection des données/i").first()).toBeVisible();
    await expect(page.locator("text=/Cookies/i")).toBeVisible();
    await expect(page.locator("text=/Conditions d'utilisation/i")).toBeVisible();
