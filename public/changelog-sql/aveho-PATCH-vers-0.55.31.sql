...
begin
  for e in
    ...
    where est_partenaire = true
      and not exists (
        select 1 from etablissements_partenaires ep
        where ep.link_to_etablissement_id = etablissements.id
      )
  loop
    ...
    values (
      ...
      e.groupement_id, e.id, coalesce(e.actif, true), coalesce(e.created_at, now())
    );
...
declare
  ...
  v_is_admin boolean;
begin
  select * into v_etab from etablissements where id = p_etab_id;
  if not found then
  ...
  left join roles r on r.id = ms.role_id
  where ms.user_id = auth.uid() and ms.structure_id = v_etab.structure_id;
  ...
  values (
    v_etab.structure_id, v_etab.nom, v_etab.type, v_etab.adresse, v_etab.cp, v_etab.ville,
    v_etab.latitude, v_etab.longitude, v_etab.groupement_id, v_etab.id, true, auth.uid()
  )
