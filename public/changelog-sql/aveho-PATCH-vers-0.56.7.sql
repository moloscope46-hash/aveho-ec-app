-- ============================================================
--  AVEHO EC — Patch 0.56.7 (version SAFE — corrigée)
--  Détection doublons forces (étabs + médecins + groupements
--  optionnels). Détection conditionnelle des groupements si
--  la table existe sur cette base.
-- ============================================================
...
do $$
begin
  if to_regclass('public.groupements') is not null then
    execute 'create index if not exists idx_groupements_nom_trgm on groupements using gin (nom gin_trgm_ops)';
  end if;
end $$;
...
create or replace function detecter_doublons_etablissements(
  ...
  etab_1_id uuid, etab_1_nom text, etab_1_siret text, etab_1_finess text, etab_1_cp text, etab_1_ville text,
  etab_2_id uuid, etab_2_nom text, etab_2_siret text, etab_2_finess text, etab_2_cp text, etab_2_ville text,
  score numeric, motif text
)
language sql security definer set search_path = public as $body$
  ...
  paires as (
    ...
    from mes_etabs e1
    join mes_etabs e2 on e1.id < e2.id
  )
  select id_1, nom_1, siret_1, finess_1, cp_1, ville_1,
...
-- RPC groupements (conditionnelle)
do $$
begin
  if to_regclass('public.groupements') is not null then
    execute $f$
      create or replace function detecter_doublons_groupements(
        ...
        grp_1_id uuid, grp_1_nom text, grp_1_nb_etabs bigint,
        grp_2_id uuid, grp_2_nom text, grp_2_nb_etabs bigint,
        score numeric
      )
      language sql security definer set search_path = public as $body$
        with mes_grp as (
          select g.id, g.nom,
            (select count(*) from etablissements where groupement_id = g.id) as nb_etabs
          from groupements g
        ...
        select id_1, nom_1, nb_1, id_2, nom_2, nb_2, score
        from paires
        where score >= p_seuil_similarite
          and not exists (
            select 1 from doublons_ignores di
    ...
    execute $f$
      ...
      language sql security definer set search_path = public as $body$
        select null::uuid, null::text, null::bigint, null::uuid, null::text, null::bigint, null::numeric where false;
      $body$;
    $f$;
    raise notice 'Table groupements absente — stub vide créé';
  end if;
end $$;
...
create or replace function detecter_doublons_medecins(
  ...
  med_1_id uuid, med_1_nom text, med_1_prenom text, med_1_rpps text, med_1_specialite text, med_1_ville text, med_1_nb_prescriptions int,
  med_2_id uuid, med_2_nom text, med_2_prenom text, med_2_rpps text, med_2_specialite text, med_2_ville text, med_2_nb_prescriptions int,
  score numeric, motif text
)
language sql security definer set search_path = public as $body$
  with mes_med as (
  ...
  select id_1, nom_1, prenom_1, rpps_1, spe_1, ville_1, nb_1,
...
begin
  if p_cible = 'etablissement' then
    ...
    if to_regclass('public.groupements') is null then
      raise exception 'Table groupements absente';
    end if;
    execute 'select structure_id from groupements where id = $1' into v_struct_id using p_id_1;
  ...
  if v_struct_id is null then raise exception 'Entité introuvable'; end if;
