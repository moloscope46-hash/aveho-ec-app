-- =============================================================
-- HOTFIX-aveho-0.61.7-ABSOLU.sql
-- TROIS ÈME tentative : on ajoute created_at + updated_at explicitement
-- via ALTER pour TOUTES les tables (au cas où elles existent avec un autre schéma).
-- 100% idempotent. Vraiment.
-- =============================================================
...
ALTER TABLE bilans_sav_articles
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
...
-- =====================================================================
-- BLOC 3 : Vue v_analytics_sav (SANS se.created_at car certaines tables n'en ont pas)
-- =====================================================================
...
SELECT
  ...
  se.verdict, se.executee_at, se.structure_id, se.magasin_id,
  COALESCE(se.executee_at, NOW()) AS created_at,
  bs.nom AS bilan_nom,
...
SELECT 'sav_executions cols' AS info, column_name FROM information_schema.columns
WHERE table_name = 'sav_executions' ORDER BY ordinal_position;
