...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 0.65.1 hotfix3 : fallback robuste à 3 niveaux (toute erreur 400/42703 dégrade)
      // Niveau 1 : avec roles join + permissions_json + droits
      let mRes = await supabase.from("membres_structure")
        .select("structure_id, role, structures(nom), roles(id,nom,systeme,permissions_json,droits,icone,couleur)")
        .limit(1).maybeSingle();
      // Niveau 2 : avec roles join sans permissions_json/droits
      if (mRes.error) {
        mRes = await supabase.from("membres_structure")
          .select("structure_id, role, structures(nom), roles(id,nom,systeme,icone,couleur)")
          .limit(1).maybeSingle();
      }
      // Niveau 3 : SANS le join roles (table roles peut ne pas exister ou pas de FK)
      if (mRes.error) {
        mRes = await supabase.from("membres_structure")
          .select("structure_id, role, structures(nom)")
          .limit(1).maybeSingle();
      }
      // Niveau 4 : absolument minimaliste (sans structures non plus)
      if (mRes.error) {
        mRes = await supabase.from("membres_structure")
