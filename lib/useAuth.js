...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 0.62.98 : split jointure PostgREST (FK pas toujours déclarée → 400)
      // 0.62.99 : skip silencieux si table absente (404) — mémoire de session
      ...
      if (!cur && structureId && !skipMM) {
        try {
          const res = await supabase
            ...
            .limit(1)
            .maybeSingle();
          // Si erreur 404 ou autre : mémoriser pour ne plus retenter cette session
          if (res.error && (res.error.code === "PGRST116" || res.error.code === "42P01" || res.error.message?.includes("not found") || res.error.message?.includes("does not exist"))) {
            try { sessionStorage.setItem(SKIP_MM_KEY, "1"); } catch {}
          } else if (res.data?.magasin_id) {
            const { data: magasin } = await supabase
              .from("magasins")
              .select("id, etablissement_id, nom")
              .eq("id", res.data.magasin_id)
              .maybeSingle();
            const magasinEtabId = magasin?.etablissement_id;
            if (magasinEtabId) {
          ...
          // Erreur réseau ou table absente : mémoriser pour cette session
          try { sessionStorage.setItem(SKIP_MM_KEY, "1"); } catch {}
        }
      }
