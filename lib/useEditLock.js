...
export function useEditLock(resourceType, resourceId, enabled = true) {
  ...
  // 0.65.1 hotfix3 : cache "table absente" pour éviter spam de 404
  ...
  const tryAcquireLock = useCallback(async () => {
    ...
    if (tableMissingRef.current) {
      // Table edit_locks absente, on désactive silencieusement
    ...
    try {
      ...
      // Si la table n'existe pas (404 / 42P01), on cache et on stoppe
      if (selErr && (selErr.code === "42P01" || selErr.message?.includes("does not exist") || selErr.message?.includes("schema cache"))) {
