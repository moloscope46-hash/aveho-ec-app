...
import { isEditLocksDisabled, markEditLocksMissing, probeEditLocksTable } from "./editLocksGuard";
...
export function useEditLock(resourceType, resourceId, enabled = true) {
  ...
  // 0.65.1 hotfix4 : guard GLOBAL — si la table n'existe pas, on n'envoie AUCUNE requête
  ...
  const tryAcquireLock = useCallback(async () => {
    ...
    if (tableMissingRef.current || isEditLocksDisabled()) {
      setLocked(false); setChecking(false);
      return false;
    }
    // Probe la table 1 seule fois avant toute requête
    ...
    try {
      const { data: existingLocks, error: selErr } = await supabase
        .from("edit_locks")
        .select("*")
      ...
      if (selErr && (selErr.code === "42P01" || selErr.code === "PGRST116" || selErr.message?.includes("does not exist") || selErr.message?.includes("schema cache"))) {
