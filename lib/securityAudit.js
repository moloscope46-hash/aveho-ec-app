...
export async function logSecurityEvent(supabase, ctx, eventType, details = {}) {
  ...
  // 0.58.2 : Fix bug 403 audit_log RLS
  //
  // Le RLS sur audit_log exige soit user_id = auth.uid(), soit une appartenance
  // à structure_id. Pour les events sécurité où on n'a pas encore l'auth
  // (ex: login échoué pour email inconnu), structure_id ET user_id sont null
  // → RLS rejette à 403 → console pollutée.
  //
  // Stratégie : skip l'insert silencieusement si pas d'auth suffisante.
  // On perd les logs côté Supabase pour ces cas, MAIS :
  //  - Les login échoués sont déjà loggés côté Supabase Auth logs (Dashboard)
  //  - Le rate-limit côté client (0.57.36) bloque les bruteforce
  //  - Le honeypot (0.57.38) détecte les bots
  // Donc on ne perd rien de critique, et on évite les 403 en cascade.
  //
  // Pour les events où on a bien un user (LOGIN_SUCCESS, MFA_*, ACCESS_DENIED, etc.),
  // ctx.userId est rempli → l'insert passe normalement.

  if (!ctx?.userId) {
    // Pas d'user identifié → log côté console only (analyse dev)
    // Ne PAS tenter l'insert qui pollue avec des 403
    if (typeof window !== "undefined" && window.location?.hostname === "localhost") {
      logger.warn(`[securityAudit] ${eventType} skipped (no auth):`, ctx?.userEmail || "anonymous");
    }
    return;
  }
  ...
  try {
    ...
    if (error) {
      // RLS ou autre erreur → log warn mais ne crash pas
      logger.warn(`[securityAudit] ${eventType} rejected:`, error.message);
    }
  } catch (e) {
