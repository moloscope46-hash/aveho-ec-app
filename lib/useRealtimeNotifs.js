  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      try {
        ...
        // === 3. Signalements critiques (0.65.16 : try/catch car criticite peut ne pas exister) ===
        try {
          const { data: signs, error: signsErr } = await supabase
            .from("signalements")
            .select("id, titre, statut, type")
            .eq("structure_id", auth.structureId)
            .gt("created_at", lastChecksRef.current.signalements)
            .eq("statut", "Nouveau");
          if (!signsErr && signs && signs.length > 0) {
            const { notify } = await import("../lib/notifSounds");
            signs.forEach(s => {
              notify(
                `⚠️ Signalement ${s.type || "Nouveau"}`,
    ...
    async function checkSLA() {
      if (!alive) return;
      try {
        // 0.65.17 : etat → statut + Résolue → Clôturée + try/catch
        const { data: intervs, error } = await supabase
          .from("interventions")
          .select("id, numero, description, urgence, statut, created_at, technicien_user_id")
          .eq("structure_id", auth.structureId)
          .not("statut", "in", '("Clôturée","Refusée")');
        if (error || !intervs || intervs.length === 0) return;
