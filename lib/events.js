...
export async function logEvent(supabase, auth, ev) {
  ...
  try {
    ...
    console.warn("logEvent - audit échoué :", e);
  }
  ...
  if (ev.notif === true || ev.notifType) {
    ...
    try {
      ...
      console.warn("logEvent - notif échouée :", e);
    }
    ...
    if (ev.push || ev.webhook || ev.notif === true) {
      ...
      }).catch((e) => console.warn("send-push:", e?.message));
      ...
      }).catch((e) => console.warn("send-webhook:", e?.message));
